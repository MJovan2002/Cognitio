...
impl<G> AdaDelta<G> {
    pub fn new(learning_rate: f64, gamma: f64) -> Self {
        Self { learning_rate, gamma, g: None }
    }
}

impl<G: Iterable + Zero> Optimizer<G> for AdaDelta<G> {
    fn gradients_to_deltas(&mut self, mut gradients: G) -> Option<G> {
        let mut g = self.g.take().unwrap_or(gradients.zero());
        gradients.iterate(&mut g, self.learning_rate, self.gamma);
        self.g = Some(g);
        Some(gradients)
    }
}
...
trait Iterable: Sized {
    fn iterate(&mut self, g: &mut Self, learning_rate: f64, gamma: f64);
}

impl Iterable for [Void; 0] {
    fn iterate(&mut self, _: &mut Self, _: f64, _: f64) {}
}
...
impl<T: Float + From<f64>, B: BackendProvider, const N: usize> Iterable for Tensor<T, B, N> {
    fn iterate(&mut self, g: &mut Self, learning_rate: f64, gamma: f64) {
        self.iter_mut().zip(g.iter_mut())
            .for_each(|(t, g)| {
                *g = T::from(gamma) * *g * *g + T::from(1. - gamma) * *t * *t;
...
impl<T: Float + From<f64>, B: BackendProvider, const N: usize, const M: usize> Iterable for [Tensor<T, B, N>; M] {
    fn iterate(&mut self, g: &mut Self, learning_rate: f64, gamma: f64) {
        self.into_iter().zip(g.into_iter()).for_each(|(t, g)| t.iterate(g, learning_rate, gamma))
    }
}

impl<A: Iterable, B: Iterable> Iterable for (A, B) {
    fn iterate(&mut self, g: &mut Self, learning_rate: f64, gamma: f64) {
        self.0.iterate(&mut g.0, learning_rate, gamma);
        self.1.iterate(&mut g.1, learning_rate, gamma);
    }
}